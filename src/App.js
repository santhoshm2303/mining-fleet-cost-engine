...
const defaultFormulas = () => [
  ...
  {key:"cycleTime",label:"Total Cycle Time",unit:"min",section:"🚛 TRUCK — Cycle & Charging",group:"Cycle",formula:"T_spotLoadQueueDump + avgLoadedTravelTime + avgUnloadedTravelTime + avgTkphDelay"},
...
export default function App(){
  ...
  const navGroups=[
    ...
    {label:"Setup",items:[{id:"mapping",label:"Field Mapping",icon:"🔗"},{id:"formulas",label:"Formulas",icon:"🧮"},{id:"fleets",label:"Fleet Combos",icon:"🏗️"}]},
    ...
    {label:"Compare",items:[{id:"comparison",label:"Comparison",icon:"⚖️"}]},
  ];
      ...
      <div style={{padding:"20px 32px 60px",maxWidth:1600,margin:"0 auto"}}>
        ...
        {page==="formulas"&&(<div>
          ...
          <div style={{...cardS,overflowX:"auto"}}><table style={{borderCollapse:"collapse",fontFamily:ff,fontSize:12,width:"100%"}}>
            ...
            <tbody>{(()=>{let lS=null,lG=null,rn=0;return formulas.filter(f=>{if(!formulaSearch)return true;const s=formulaSearch.toLowerCase();return[f.key,f.label,f.formula,f.section||"",f.group||""].some(x=>x.toLowerCase().includes(s))}).flatMap((f,i)=>{
              const rows=[];
              if(f.section&&f.section!==lS){lS=f.section;lG=null;rows.push(<tr key={`s${i}`}><td colSpan={7} style={{padding:"14px 10px 6px",color:P.pri,fontWeight:700,fontSize:13,borderBottom:`2px solid ${P.pri}`,background:P.secBg}}>{f.section}</td></tr>)}
              if(f.group&&f.group!==lG){lG=f.group;rows.push(<tr key={`g${i}`}><td colSpan={7} style={{padding:"7px 10px 3px 22px",color:P.txD,fontWeight:600,fontSize:11,borderBottom:`1px solid ${P.bd}`,background:"#f8fafc"}}>▸ {f.group}</td></tr>)}
