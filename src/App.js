...
export default function App(){
  ...
  const addManP=()=>updScn(s=>({...s,manualData:[...s.manualData,{period:s.manualData.length+1,periodLabel:`P${s.manualData.length+1}`,days:91,hours:2184,oreMined:0,wasteMined:0,totalMined:0,totalRampMined:0,avgLoadedTravelTime:10,avgUnloadedTravelTime:8,avgTkphDelay:0,avgNetPower:150}]}));
      ...
      <div style={{padding:"20px 32px 60px",maxWidth:1600,margin:"0 auto"}}>
        ...
        {page==="schedule"&&(<div>
          ...
          <div style={{...cardS,padding:18,marginBottom:18}}>
            ...
            {scn.csvData&&<p style={{color:P.gn,fontSize:12,marginTop:8,fontWeight:600}}>✓ {scn.csvData.np} periods · {scn.csvRawLabels.length} rows. Configure in Field Mapping tab.</p>}
          </div>
          {!scn.csvData&&(<div>
            <div style={{...cardS,overflowX:"auto"}}><table style={{borderCollapse:"collapse",fontFamily:ff,fontSize:12,width:"100%"}}>
              <thead><tr style={{background:P.secBg,borderBottom:`2px solid ${P.bdS}`}}>
                {["#","Period","Days","Hrs","Ore","Waste","Total","LoadTT","UnloadTT","TKPH","NetPwr"].map((h,i)=>(<th key={i} style={{...thS,textAlign:i>3?"right":"left"}}>{h}</th>))}
                <th/>
              </tr></thead>
              <tbody>{scn.manualData.map((p,idx)=>(<tr key={idx} style={{borderBottom:`1px solid ${P.bd}`,background:idx%2?P.input+"55":"transparent"}}>
                <td style={{padding:"6px 8px",color:P.txD}}>{p.period}</td>
                {[["periodLabel","t"],["days","n"],["hours","n"],["oreMined","n"],["wasteMined","n"],["totalMined","n"],["avgLoadedTravelTime","n"],["avgUnloadedTravelTime","n"],["avgTkphDelay","n"],["avgNetPower","n"]].map(([k,t])=>(<td key={k} style={{padding:"4px 5px"}}><input type={t==="t"?"text":"number"} value={p[k]} onChange={e=>updManP(idx,k,t==="t"?e.target.value:parseFloat(e.target.value)||0)} style={{width:t==="t"?70:75,padding:"4px 6px",background:P.input,border:`1px solid ${P.bd}`,borderRadius:5,color:k==="totalMined"?P.pri:P.tx,fontFamily:mf,fontSize:11,textAlign:t==="t"?"left":"right",fontWeight:k==="totalMined"?700:400}}/></td>))}
                <td>{scn.manualData.length>1&&<button onClick={()=>updScn(s=>({...s,manualData:s.manualData.filter((_,i)=>i!==idx)}))} style={{background:P.rdBg,border:`1px solid ${P.rd}22`,borderRadius:5,color:P.rd,cursor:"pointer",padding:"2px 8px"}}>×</button>}</td>
              </tr>))}</tbody>
            </table></div>
        ...
        {page==="charts_results"&&(<div>
          ...
          {results.length===0?<p style={{color:P.txD}}>No results data.</p>:(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
            <div style={cardS}><div style={{padding:"16px 16px 4px",fontWeight:700,color:P.pri,fontSize:13}}>Cost per Tonne by Period</div>
              <ResponsiveContainer width="100%" height={300}><LineChart data={results.filter(function(r){return r.res}).map(function(r){return{period:r.periodLabel,TotalCPT:r.res.totPerT||0,TruckCPT:r.res.trkPerT||0,DiggerCPT:r.res.digOpxPerT||0}})} margin={{top:10,right:20,left:10,bottom:40}}>
                ...
                <Legend wrapperStyle={{fontSize:11}}/><Line type="monotone" dataKey="TotalCPT" stroke={mClr[0]} strokeWidth={2} name="Total $/t"/><Line type="monotone" dataKey="TruckCPT" stroke={mClr[1]} strokeWidth={2} name="Truck $/t"/><Line type="monotone" dataKey="DiggerCPT" stroke={mClr[2]} strokeWidth={2} name="Digger $/t"/>
            ...
            <div style={cardS}><div style={{padding:"16px 16px 4px",fontWeight:700,color:P.pri,fontSize:13}}>Total Cost Breakdown (Stacked)</div>
              <ResponsiveContainer width="100%" height={300}><BarChart data={results.filter(function(r){return r.res}).map(function(r){return{period:r.periodLabel,TruckOpex:r.res.totTrkExc||0,DiggerOpex:r.res.digOpxTotal||0,Rehandle:r.res.digRehandle||0}})} margin={{top:10,right:20,left:10,bottom:40}}>
                ...
                <Legend wrapperStyle={{fontSize:11}}/><Bar dataKey="TruckOpex" stackId="a" fill={mClr[0]} name="Truck exc Cpx"/><Bar dataKey="DiggerOpex" stackId="a" fill={mClr[1]} name="Digger Opex"/><Bar dataKey="Rehandle" stackId="a" fill={mClr[2]} name="Rehandle"/>
              </BarChart></ResponsiveContainer>
            </div>
            <div style={cardS}><div style={{padding:"16px 16px 4px",fontWeight:700,color:P.pri,fontSize:13}}>Fleet Sizing by Period</div>
              <ResponsiveContainer width="100%" height={300}><BarChart data={results.filter(function(r){return r.res}).map(function(r){return{period:r.periodLabel,Trucks:r.res.trkReqR||0,Diggers:r.res.digFleet||0,Chargers:r.res.chgStaRnd||0}})} margin={{top:10,right:20,left:10,bottom:40}}>
