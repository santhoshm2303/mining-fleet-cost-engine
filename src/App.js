...
// ─── LOCAL STORAGE ─────────────────────────────────────────────────────
var SAVE_KEY="mfce_state_v5";
var SAVE_VER=5;
function saveToLS(data){try{data._ver=SAVE_VER;localStorage.setItem(SAVE_KEY,JSON.stringify(data))}catch(e){}}
function loadFromLS(){try{var d=localStorage.getItem(SAVE_KEY);if(!d)return null;var p=JSON.parse(d);if(!p||typeof p!=="object"||p._ver!==SAVE_VER){localStorage.removeItem(SAVE_KEY);return null}return p}catch(e){localStorage.removeItem(SAVE_KEY);return null}}
  ...
  render() {
    if (this.state.error) {
      return React.createElement("div", {style:{padding:40,fontFamily:"sans-serif"}},
        React.createElement("h1", {style:{color:"#c93131"}}, "Something went wrong"),
        React.createElement("pre", {style:{background:"#fef2f2",padding:20,borderRadius:8,fontSize:12,overflow:"auto",marginTop:16}}, String(this.state.error)),
        React.createElement("button", {onClick:function(){localStorage.clear();window.location.reload()},style:{marginTop:16,padding:"10px 20px",background:"#1d4ed8",color:"#fff",border:"none",borderRadius:6,cursor:"pointer",fontSize:14}}, "Clear Data & Reload")
      );
    }
...
function AppInner(){
  ...
  const [trucks,setTrucks]=useState(function(){try{if(saved&&Array.isArray(saved.trucks)&&saved.trucks.length>0)return saved.trucks}catch(e){}return[mkTruck(),mkTruckL()]});
  const [diggers,setDiggers]=useState(function(){try{if(saved&&Array.isArray(saved.diggers)&&saved.diggers.length>0)return saved.diggers}catch(e){}return[mkDigger(),mkDigger4()]});
  const [otherA,setOtherA]=useState(function(){try{if(saved&&saved.otherA&&typeof saved.otherA==="object")return saved.otherA}catch(e){}return defaultOther()});
  const [formulas,setFormulas]=useState(function(){try{if(saved&&Array.isArray(saved.formulas)&&saved.formulas.length>0)return saved.formulas}catch(e){}return defaultFormulas()});
  const [fleets,setFleets]=useState(function(){try{if(saved&&Array.isArray(saved.fleets)&&saved.fleets.length>0)return saved.fleets}catch(e){}return[mkFleet("Fleet 1",0,0),mkFleet("Fleet 2",1,1)]});
  const [scenarios,setScenarios]=useState(function(){try{if(saved&&Array.isArray(saved.scenarios)&&saved.scenarios.length>0)return saved.scenarios}catch(e){}return[mkScenario("Scenario ST"),mkScenario("Scenario LT")]});
